...
class Double_DQN():
    ...
    def process_data(self):
        ...
        # 取出数据，这里s和s_是需要输入到神经网络的，所以我们把数组，变成array。
        ...
        # [敲黑板]
        # 计算Double的target
        # 这时y表示s状态下的q值，返回的时候表示target(y、label都可以说)，ndarray类型
        y = self.Q_network(np.array(s, dtype='float32'))
        y = y.numpy()
        # Q-N提供action最大Q值的位置，T-N提供这个action的Q值
    ...
    def train(self, episode):
        ...
        for ep in range(episode):
            ...
            while True:
                ...
                # 进行游戏
                a = self.get_action(s)  #步骤2，获得最大Q值动作
                s_, r, done, _ = self.env.step(a) #步骤3，获得新状态，reward,结束标记done
                total_reward += r
                step += 1
